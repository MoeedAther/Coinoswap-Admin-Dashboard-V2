...
const SettingExchange = () => {
  ...
  const [visibility, setVisibility] = useState<Record<string, 'fixed' | 'floating'>>({});
  ...
  const toggleVisibility = (exchange: string, type: 'fixed' | 'floating') => {
    setVisibility({ ...visibility, [exchange]: type });
  };

  const toggleGiveaway = (exchange: string) => {
